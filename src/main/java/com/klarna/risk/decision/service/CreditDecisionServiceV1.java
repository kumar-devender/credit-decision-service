...
public class CreditDecisionServiceV1 {
    ...
    public CreditRequestDecisionV1 handleCreditRequestV1(CreditRequestV1 creditRequestV1) {
        ...
        creditHistoryRepository.addTransaction(creditRequestV1.getEmail(), buildTransaction(creditDecision.getReason(), creditRequestV1.getPurchaseAmount()));

        return CreditRequestDecisionV1.from(creditDecision);
    }

    private CustomerTransaction buildTransaction(String reason, int amount) {
        return new CustomerTransaction(reason, amount);
    }
